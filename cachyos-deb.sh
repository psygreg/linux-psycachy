...
# Function to configure LLVM LTO -- disable due to inconsistencies in compile
#configure_llvm_lto() {
...
EOF
    ...
    if [ "$_llvm_lto_selection" == "thin" ] || [ "$_llvm_lto_selection" == "full" ]; then
        ...
        # make CC=clang LD=ld.lld LLVM=1 LLVM_IAS=1 olddefconfig -- llvm is currently borked
    else
        make olddefconfig
    fi
    ...
    if [ "$_kv_name" == "$_kv_latest" ]; then
        #if [ "$_llvm_lto_selection" == "thin" ] || [ "$_llvm_lto_selection" == "full" ]; then
            # make CC=gcc bindeb-pkg -j"$(($(nproc) - 1))" LOCALVERSION=-"psycachy" KDEB_PKGVERSION="$(make kernelversion)-1"
            # make CC=clang LD=ld.lld LLVM=1 LLVM_IAS=1 bindeb-pkg -j"$(($(nproc) - 1))" LOCALVERSION=-"psycachy" KDEB_PKGVERSION="$(make kernelversion)-1" -- llvm is currently borked
        #else
        ...
        #if [ "$_llvm_lto_selection" == "thin" ] || [ "$_llvm_lto_selection" == "full" ]; then
            #make CC=gcc bindeb-pkg -j"$(($(nproc) - 1))" LOCALVERSION=-"psycachy-lts" KDEB_PKGVERSION="$(make kernelversion)-1"
            #make CC=clang LD=ld.lld LLVM=1 LLVM_IAS=1 bindeb-pkg -j"$(($(nproc) - 1))" LOCALVERSION=-"psycachy-lts" KDEB_PKGVERSION="$(make kernelversion)-1" -- llvm is currently borked
        #else
            make bindeb-pkg -j"$(($(nproc) - 1))" LOCALVERSION=-"psycachy-lts" KDEB_PKGVERSION="$(make kernelversion)-1"
            #make -j$(nproc) KDEB_PKGVERSION=${_kver_stable}.psycachy-gen bindeb-pkg
        #fi
    fi
...
install_f () {
    ...
    if [ "$_kv_name" = "$_kver_lts" ]; then
        _kedition="${KERNEL_VERSION}-psycachy-lts"
    else
        _kedition="${KERNEL_VERSION}-psycachy"
    fi

    # Install compiled kernel
    sudo dpkg -i ${build_dir}/cachyos-deb/linux-image-${_kedition}_${KERNEL_VERSION}-1_amd64.deb ${build_dir}/cachyos-deb/linux-headers-${_kedition}_${KERNEL_VERSION}-1_amd64.deb ${build_dir}/cachyos-deb/linux-libc-dev-${KERNEL_VERSION}_${KERNEL_VERSION}-1_amd64.deb
...
while :; do
    ...
    CHOICE=$(whiptail --title "Kernel Configuration Menu" --menu "Choose an option" 25 78 16 \
        ...
        "3" "Configure Tick Rate" \
        "4" "Configure NR_CPUS" \
        "5" "Configure Tick Type" \
        "6" "Configure Preempt Type" \
        "7" "Configure LRU" \
        "8" "Configure Hugepages" \
        "9" "Configure System Optimizations" \
        "10" "COMPILE AND INSTALL KERNEL" \
        "11" "UPDATE KERNEL" \
        "12" "Exit" 3>&1 1>&2 2>&3)
    ...
    3) configure_tick_rate ;;
    4) configure_nr_cpus ;;
    5) configure_tick_type ;;
    6) configure_preempt_type ;;
    7) configure_lru ;;
    8) configure_hugepages ;;
    9) configure_system_optimizations ;;
    10) first_install ;;
    11) kernel_upd ;;
    12 | q) cd $HOME
            rm -rf cachyos-deb
